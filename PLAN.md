...
**Guiding principle:** Frontier authoring judgment was the expiring resource. Author judgment-dense content (skills, playbooks, rules) first; mechanical plumbing (MCP server, installer, ports) last — any model can finish those later *using the library itself*.
...
**Optional housekeeping:** ~~repo has outgrown "ideas" — renaming is safe; GitHub redirects the old URL.~~ *Done 2026-07-06: first rename; old URLs redirect. 2026-09-13: renamed to ways-of-working; old URLs redirect. Earlier names are in git history.*
