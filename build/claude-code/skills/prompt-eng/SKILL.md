---
name: prompt-eng
description: Prompts engineered like software — a clear contract with examples, tested
  against a case set before trusting, iterated on observed failures not vibes. Use
  when writing or debugging prompts, system prompts, agent instructions, or LLM-powered
  features; when a model's output quality is the problem to solve.
---
...
Tool descriptions are prompts too — the model reads them to decide when and how to call the tool, so treat them with the same discipline:
...
Any prompt that assembles tool results or repo content into context is an injection surface — see `sec-audit`'s "LLM and pipeline surfaces" category.
