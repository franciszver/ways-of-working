---
name: brainstorm
description: Structured ideation that escapes the first idea's gravity — generate
  wide without judging, force variation along named axes, then converge with explicit
  criteria and kill reasons. Use when asked to brainstorm, generate options/names/approaches,
  find alternatives to a stuck plan, or when a decision is being made with only one
