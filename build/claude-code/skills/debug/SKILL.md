---
name: debug
description: Hypothesis-driven debugging — reproduce first, localize by bisection,
  test one hypothesis at a time, fix the cause not the symptom, prove the fix. Use
  whenever anything fails — bug report, failing test, error message, stack trace,
  regression, flaky behavior, or "it worked yesterday". Applies equally to failing
  builds, configs, queries, and pipelines. A red CI run starts with `ci-triage`.
---
