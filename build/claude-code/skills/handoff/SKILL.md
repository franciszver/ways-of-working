---
name: handoff
description: Writes a compact continuation brief so a fresh session — or a cheaper
  model — resumes exactly where this one left off, without re-deriving anything. Use
  when context is running long, when ending a session mid-task, when downshifting
  work to a smaller model, or when delegating a subtask. Also defines how to RESUME
  from a handoff.
---
...
- Work is downshifting to a cheaper/smaller model (the ways-of-working ROUTING playbook governs when) — the handoff is what makes the downshift safe.
