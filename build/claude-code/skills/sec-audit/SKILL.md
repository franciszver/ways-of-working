---
name: sec-audit
description: Security review of a diff or codebase — trace untrusted data flows, report
  only high-confidence exploitable vulnerabilities with concrete exploit scenarios,
  filter the classic false-positive categories. Use when asked for a security review/audit,
  before shipping auth/payment/user-input handling code, or when handling data from
  untrusted sources.
