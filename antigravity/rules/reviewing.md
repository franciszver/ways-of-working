...
- Report `SEVERITY path:line — defect — trigger — fix`, most severe first: BLOCKER (wrong results/data loss/security/crash), MAJOR (real failure, rarer conditions), MINOR (fragile/misleading). Cap ~10. Style feedback: one line max, only when style hides a bug.
