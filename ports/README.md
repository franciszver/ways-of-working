...
These are compressions of the canonical skills, not independent documents. When a canonical skill's judgment changes (a new rule, a changed threshold), propagate here: `AGENTS.md` sections, the matching `cursor/*.mdc`, the matching `gemini/commands/*.toml`, and the Antigravity rule/workflow. The improvement-loop checklist treats cross-port drift as a defect.
