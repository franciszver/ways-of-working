...
Formats — last verified: see CHANGELOG — AGENTS.md is plain markdown by design; Cursor rules are `.mdc` with `description`/`globs`/`alwaysApply` frontmatter; Gemini CLI commands are TOML with `description` + `prompt` (`{{args}}` substitution) per [geminicli.com/docs/cli/custom-commands](https://geminicli.com/docs/cli/custom-commands/).
