...
7. LLM and pipeline surfaces (if applicable): prompt injection via repo content or tool results fed into an agent's context; CI script injection (unpinned actions, untrusted `${{ }}` interpolated into a `run:` step); SSRF from an agent-driven tool fetching a model-chosen URL — see `prompt-eng`'s "Agent and tool prompts"
