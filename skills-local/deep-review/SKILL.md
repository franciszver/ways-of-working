...
<!-- local: derived-from: skills/deep-review/SKILL.md@5e6245cf35b6 -->
...
Review the CHANGE: default scope is the branch's commits ahead of upstream PLUS uncommitted changes (or the files/target named). Then read the callers and callees of every changed function — you cannot judge a change without its context. Set depth by stakes — auth, payments, data migration, and concurrency get the full hunt; a README fix gets a proportional glance. Review the actual files on disk, never your memory of them. If the repo has a `REVIEW.md`, read it first — its severity rules and skip-paths override the defaults below.
...
Most severe first. Max 10 findings; say "further minor issues omitted" past that. Separate "must fix before merge" from "consider" — do not let a consider-tier item block the report as if it were a blocker. A verified bug the diff did NOT introduce: still report it, appended "(PRE-EXISTING)", listed after the diff's own findings. Style feedback: ONE line maximum, only if style hides a bug. If something is done well and load-bearing — a subtle lock order, a deliberate off-spec behavior — say so in one line, so a later editor doesn't "fix" it.
...
After fixes land, run `prove` on the fixed state; delegate to the `code-reviewer` subagent (Claude Code) for an independent fresh-context pass, or a fresh session elsewhere. Cleanup the hunt turns up (duplication, dead code, needless indirection) is one summary line here, not a finding — `declutter` is the sweep that applies those fixes.
